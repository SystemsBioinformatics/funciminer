library(Biostrings)
isolate<-readAAStringSet("path/lbp_orfs.faa")
metagenome<-readAAStringSet("path/THLR_T3_1.faa")
#
read <- read.table("path/lbp_ko_raw.txt",sep="\t", header=FALSE, fill = TRUE)
namestorm<-unique(as.character(read[[1]][which(as.character(read[[2]])!="")]))
isolate<-isolate[gsub(" #.*","",names(isolate))%in%namestorm]
#
metagenome_ko <- read.table("path/THLR_T3_1_KO.txt",sep="\t", header=FALSE, fill = TRUE)
...
isolate<-isolate[gsub(" #.*","",names(isolate))%in%namestorm]
#
metagenome<-readAAStringSet("path/VECX_A11_1.faa")
metagenome_kot <- read.table("path/VECX_A11_1_KO.txt",sep="\t", header=FALSE, fill = TRUE)
VECX_A11_1_KO <- read.csv2("path/VECX_A11_1_KO.txt", header=FALSE)
metagenome_ko1 <- read.csv2("path/VECX_A11_1_KO.txt",,header=FALSE)
